...
export class BridgeClient implements SocketClient {
  ...
  private notificationHandler: ((notification: { method: string; params?: Record<string, unknown> }) => void) | null =
    null
  ...
  private pendingSwitchResolve: ((result: { deviceId: string; name: string } | null) => void) | null = null
  ...
  public async setPermissionMode(mode: PermissionMode, allowedDomains?: string[]): Promise<void> {
  ...
  > {
    ...
    if (
      extensions.length === 0 ||
      (extensions.length === 1 && (!currentDeviceId || extensions[0].deviceId === currentDeviceId))
    ) {
    ...
    switch (message.type) {
      ...
      case 'extensions_list':
        ...
        if (this.pendingDiscovery) {
          ...
          this.pendingDiscovery.resolve((message.extensions as ChromeExtensionInfo[]) ?? [])
          this.pendingDiscovery = null
        }
        break
  ...
  private handleToolResult(message: Record<string, unknown>): void {
    ...
    if (pending.isTabsContext && !this.selectedDeviceId) {
      ...
      if (isError) {
        ...
        const errorContent = (normalized as { error?: { content?: unknown[] } }).error?.content
        let errorMessage = 'Unknown error'
        if (Array.isArray(errorContent)) {
          const textItem = errorContent.find((item) => typeof item === 'object' && item !== null && 'text' in item) as
            | { text?: string }
            | undefined
          if (textItem?.text) {
  ...
  private normalizeBridgeResponse(message: Record<string, unknown>): Record<string, unknown> {
  ...
  private mergeTabsResults(results: unknown[]): unknown {
    ...
    for (const result of results) {
      ...
      const resultData = msg.result as { content?: Array<{ type: string; text?: string }> } | undefined
  ...
  private scheduleReconnect(): void {
    ...
    const delay = Math.min(2000 * Math.pow(1.5, this.reconnectAttempts - 1), 30_000)
