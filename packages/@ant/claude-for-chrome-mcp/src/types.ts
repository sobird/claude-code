...
export interface ClaudeForChromeContext {
  ...
  trackEvent?: <K extends string>(eventName: K, metadata: Record<string, unknown> | null) => void
...
export interface SocketClient {
  ...
  callTool(name: string, args: Record<string, unknown>, permissionOverrides?: PermissionOverrides): Promise<unknown>
  isConnected(): boolean
  disconnect(): void
  setNotificationHandler(handler: (notification: { method: string; params?: Record<string, unknown> }) => void): void
  /** Set permission mode for the current session. Only effective on BridgeClient. */
  setPermissionMode?(mode: PermissionMode, allowedDomains?: string[]): Promise<void>
