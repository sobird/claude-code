import type { ComputerExecutor, InstalledApp, ScreenshotResult } from './executor.js'
...
export interface ComputerUseHostAdapter {
   ...
   */
  cropRawPatch(jpegBase64: string, rect: { x: number; y: number; width: number; height: number }): Buffer | null
}
...
export interface ComputerUseSessionContext {
  ...
  onTeachPermissionRequest?(req: CuTeachPermissionRequest, signal: AbortSignal): Promise<CuPermissionResponse>
...
export interface ComputerUseOverrides {
   ...
   */
  onTeachPermissionRequest?: (req: CuTeachPermissionRequest) => Promise<CuPermissionResponse>
