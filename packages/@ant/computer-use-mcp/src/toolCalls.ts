...
import type { ComputerExecutor, DisplayGeometry, InstalledApp, ScreenshotResult } from './executor.js'
...
function requireNumber(args: Record<string, unknown>, key: string): number | Error {
  const v = args[key]
...
function requireString(args: Record<string, unknown>, key: string): string | Error {
  const v = args[key]
  if (typeof v !== 'string') {
 ...
 */
function extractCoordinate(args: Record<string, unknown>, paramName: string = 'coordinate'): [number, number] | Error {
...
function tierSatisfies(grantTier: CuAppPermTier | undefined, actionKind: CuActionKind): boolean {
...
async function runInputActionGates(
  ...
  const tierByBundleId = new Map(overrides.allowedApps.map((a) => [a.bundleId, a.tier] as const))
...
async function releaseHeldMouse(adapter: ComputerUseHostAdapter): Promise<void> {
...
async function handleRequestAccess(
  ...
  const { needDialog, skipDialogGrants, willHide, tieredApps, userDenied, policyDenied } = await buildAccessRequest(
    adapter,
    apps,
 ...
 */
function buildUserDeniedGuidance(userDenied: Array<{ requestedName: string; displayName: string }>): string {
 ...
 * another approach.
 */
function buildPolicyDeniedGuidance(policyDenied: Array<{ requestedName: string; displayName: string }>): string {
...
async function handleTeachBatch(
  ...
  for (const [i, raw] of rawSteps.entries()) {
    ...
    const v = await validateTeachStepArgs(raw as Record<string, unknown>, adapter, overrides, `steps[${i}]`)
 ...
 */
function uniqueDisplayLabels(displays: readonly DisplayGeometry[]): Map<number, string> {
...
async function buildMonitorNote(
  ...
  const othersList =
    others.length > 0 ? ` Other attached monitors: ${others.map((n) => `"${n}"`).join(', ')}.` + switchHint : ''
...
async function handleZoom(
  ...
  if (x1 <= x0) return errorResult('region x1 must be greater than x0', 'bad_args')
  if (y1 <= y0) return errorResult('region y1 must be greater than y0', 'bad_args')
  ...
  if (x1 > last.width || y1 > last.height) {
    return errorResult(`region exceeds screenshot bounds (${last.width}×${last.height})`, 'bad_args')
  }
...
async function handleClickVariant(
  ...
  const clickActionKind: CuActionKind =
    button !== 'left' || (modifiers !== undefined && modifiers.length > 0) ? 'mouse_full' : 'mouse'
...
async function handleKey(
  ...
  if (keySequence instanceof Error) return errorResult('text is required', 'bad_args')
...
async function handleDrag(
  ...
  if (endCoord instanceof Error) return errorResult(endCoord.message, 'bad_args')
  ...
  if (args.start_coordinate !== undefined) {
    ...
    if (startCoord instanceof Error) return errorResult(startCoord.message, 'bad_args')
    rawFrom = startCoord
  }
  ...
  const from =
    rawFrom === undefined
      ? undefined
      : scaleCoord(rawFrom[0], rawFrom[1], overrides.coordinateMode, display, overrides.lastScreenshot, adapter.logger)
...
function handleListGrantedApplications(overrides: ComputerUseOverrides): CuCallToolResult {
  return okJson({
 ...
 */
async function handleWait(args: Record<string, unknown>): Promise<CuCallToolResult> {
...
async function handleLeftMouseUp(
  ...
  const releaseFirst = async (err: CuCallToolResult): Promise<CuCallToolResult> => {
    await adapter.executor.mouseUp()
    mouseButtonHeld = false
    mouseMoved = false
    return err
...
export async function handleToolCall(
  ...
  const overrides: ComputerUseOverrides = rawOverrides.allowedApps.some(
    (a) => a.tier === undefined || userDeniedSet.has(a.bundleId) || isPolicyDenied(a.bundleId, a.displayName),
  )
    ? {
        ...rawOverrides,
        allowedApps: rawOverrides.allowedApps
          ...
          .map((a) => (a.tier !== undefined ? a : { ...a, tier: getDefaultTierForApp(a.bundleId, a.displayName) })),
      }
    : rawOverrides
